...
if ENV:
    ...
    MONGO_DB_URI = os.environ.get("MONGO_DB_URI", "mongodb+srv://SophiaMusic:zNGvnNT9mbe5XZnC@cluster0.sn1ilu6.mongodb.net/?retryWrites=true&w=majority")
    NO_LOAD = os.environ.get("NO_LOAD", "").split()
    START_IMG = os.environ.get(
        "START_IMG", "https://te.legra.ph/file/b0f1eedd304ed8808f889.jpg"
    )
    STRICT_GBAN = bool(os.environ.get("STRICT_GBAN", True))
    SUPPORT_CHAT = os.environ.get("SUPPORT_CHAT", "+szs3WEBqeNU3ZjE1")
    TEMP_DOWNLOAD_DIRECTORY = os.environ.get("TEMP_DOWNLOAD_DIRECTORY", "./")
    TOKEN = os.environ.get("TOKEN", "5998139371:AAE9TXyrbJpODDPC95sICPBNtYYJxbR-WVk")
    ...
    try:
        DRAGONS = set(int(x) for x in os.environ.get("DRAGONS","5360305806 6109442416 736041718").split())
        DEV_USERS = set(int(x) for x in os.environ.get("DEV_USERS","5360305806 6109442416 736041718 5916859256").split())
    except ValueError:
        raise Exception("Your sudo or dev users list does not contain valid integers.")

    try:
        DEMONS = set(int(x) for x in os.environ.get("DEMONS","5360305806 6109442416 736041718").split())
    except ValueError:
        raise Exception("Your support users list does not contain valid integers.")

    try:
        TIGERS = set(int(x) for x in os.environ.get("TIGERS","5360305806 6109442416 736041718").split())
    except ValueError:
        raise Exception("Your tiger users list does not contain valid integers.")

    try:
        WOLVES = set(int(x) for x in os.environ.get("WOLVES", "5360305806 6109442416 736041718").split())
    except ValueError:
